class ExamReservationsController < ApplicationController
  ...
  def create
    ...
    exam_reservation = ExamReservation.find_by(user_id: user.id, exam_schedule_id: exam_schedule_id)
    raise Errors::InvalidRequest.new(Errors::ALREADY_RESERVED_EXAM_SCHEDULE_MESSAGE) if exam_reservation.present?

    new_exam_reservation = ExamReservation.create!(user_id: user.id, exam_schedule_id: exam_schedule_id, is_confirmed: false)
